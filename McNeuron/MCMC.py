...
class MCMC(object):
    ...
    def set_ext_red_list(self, neuron):
        ...
        J = np.array([])
        for i in I:
            if branch_order[neuron.parent_index[I[i]]] == 1:
                J = np.append(J, i)
        J = np.array(J, dtype=int)
        ext_red_list[2, J] = 1
        ext_red_list.astype(int)
        ext_red_list[:, 0:self.n_soma] = 0
        return ext_red_list
    ...
    def do_ext_red_end_points(self, neuron):
        a = self.ext_red_list(neuron)[1:3, :]
    ...
    def show_MCMC(self,start,size_x,size_y):
        ...
        plt.legend(self.list_features,bbox_to_anchor=(3.2,0))
        plt.subplot(2,2,4)
        plt.plot(np.convolve(self.acc, np.ones(30))/30)
