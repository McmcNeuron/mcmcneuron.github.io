...
class Neuron(object):
    """
    Main class for Neurons. This class represents the neuron by a list of
    `Node`s. Borrowed from swc format, each node indicates a point on the
    neuron. each node has parent and children (at most two children) and set
    of all node with their parents make a tree structure; a connected graph
    without loop. The Soma represents by a few nodes and one of them is called
    root node and it's decendent of all the nodes in the neuron (including
    other soma nodes). Notice that all nodes from index 0 to index of `n_soma`
    in the `nodes_list` are soma. This class contains the attributes to
    calculate different features of the neuron. The iput file can be a swc file
    or the list of nodes. all indexing for not-soma nodes (or the nodes that
    are very close to the soma) coming from self.nodes_list
    ...
    child_index : array of shape = [2, n_node]
        first row: The index of first child of the node ( the ordering of the
        nodes are arrbitraty). second row: nan if neuron is order one and
        index of second child of the node if it's a branching node.
    ...
    def __init__(self,
        ...
        self.turnicate = 100
        if input_format == 'swc':
            self.read_swc_matrix(np.loadtxt(input_file))
            self.fit()

        if input_format == 'swc without attributes':
            self.read_swc_matrix(np.loadtxt(input_file))

        if input_format == 'only list of nodes':
    ...
    def set_features(self):
        ...
        Given:
            ...
            location
        """
        branch_order = self.branch_order(self.parent_index)
        ...
        value_dis, value_avoid = \
            self.set_discrepancy(np.arange(2, 120))
        ...
        main_branch_depth, _, main_dead_depth, _, _, _ = \
            self.type_at_depth(self.branch_order(main_parent), main_parent)
        ...
        self.features['mean neuronal/euclidean'] = \
        ...
        self.features['discrepancy space'] = value_dis
        self.features['self avoidance'] = value_avoid
        self.features['pictural image xy'] = self.set_pictural_xy(10., 10.)
        ...
        self.features['toy'] = \
            np.array([np.sqrt(((self.features['neuronal/euclidean for segments'] - 1.04)**2).mean())])
    ...
    def get_neural_and_euclid_lenght_main_point(self, branch_order, distance_from_parent):
        (main_node,) = np.where(branch_order[self.n_soma:] != 1)
        main_node += self.n_soma
        main_node = np.append(0, main_node)
        sink_main = deepcopy(self.parent_index)
        sink_main[main_node] = 0

        distance_from_parent_sink_for_main = deepcopy(distance_from_parent)
        distance_from_parent_sink_for_main[main_node] = 0

        distance_from_main = np.zeros(self.n_node)

        filt = np.zeros(self.n_node)
        filt[main_node] = main_node
        parent_main_node = np.zeros(self.n_node)
        up = np.arange(self.n_node)
        while(sum(up) != 0):
            distance_from_main += distance_from_parent_sink_for_main[up]
            parent_main_node += filt[up]
            up = sink_main[up]

        neural = distance_from_main[self.parent_index]
        neural += distance_from_parent
        neural = neural[main_node[1:]]
        parent_main_node = parent_main_node[self.parent_index[main_node]]
        parent_main_node = parent_main_node.astype(int)
    ...
    def branching_type(self, main_node, parent_main_node):
        ...
        branch_order_main_point = self.branch_order(parent_branch_main_point_transform)
    ...
    def discrepancy(self, x_mesh, y_mesh, z_mesh):
        ...
        return len(index), (count>1).sum()

    def set_pictural_xy(self, x_mesh, y_mesh):
        """
        Calculate the number of nodes in a 2d equidistance mesh.
        """
        image = np.zeros(int(x_mesh * y_mesh))
        X = self.normlize(self.location[0, :], x_mesh-1)
        Y = self.normlize(self.location[1, :], y_mesh-1)
        L = X + x_mesh*Y
    ...
    def set_pictural_xyz(self, x_mesh, y_mesh, z_mesh):
        """
        Calculate the number of nodes in a 2d equidistance mesh.
        """
        image = np.zeros(int(x_mesh * y_mesh * z_mesh))
        X = self.normlize(self.location[0, :], x_mesh-1)
        Y = self.normlize(self.location[1, :], y_mesh-1)
        Z = self.normlize(self.location[2, :], z_mesh-1)
        L = X + x_mesh*Y + x_mesh*y_mesh*Z
        L = np.array(L).astype(int)
        index, count = np.unique(L, return_counts=True)
        image[list(index)] = list(count)
    ...
    def set_discrepancy(self, mesh):
        value_dis = np.zeros(len(mesh))
        value_avoid = np.zeros(len(mesh))
        for i in range(len(mesh)):
            a = float(mesh[i])
            b, c = self.discrepancy(a, a, a)
            value_dis[i] = b
            value_avoid[i] = c
        value_dis = value_dis/sum(value_dis)
        value_avoid = value_avoid/sum(value_avoid)
        return value_dis, value_avoid

    def branch_order(self, parent_index):
        """
    ...
    def type_at_depth(self, branch_order, parent_index, branch_branch=[], branch_die=[], die_die=[]):
        dead_index = np.where(branch_order == 0)[0]
        continue_index = np.where(branch_order == 1)[0]
        branch_index = np.where(branch_order == 2)[0]
        depth_all = self.dendogram_depth(parent_index)
    ...
    def read_swc_matrix(self, swc_matrix):
        ...
        self.n_node = len(self.nodes_type)
    ...
    def _set_showing_hist_legends(self):
        ...
        'distance_from_root': 30, # np.arange(0,1500,10),
        ...
        'ratio_neural_euclidian_important': np.arange(1,3,.05),
        'ratio_euclidian_neuronal' : np.arange(1,3,.05),
        'distance_from_parent' : 30, # np.arange(0,60,1),
...
class Node(object):
    ...
    each node has parent (another node), children (None, one or more nodes),
    radius, Euclidian cordinates and type
    ...
    def __init__(self):
        ...
        self.node_type = None # it can be soma, dendrite, axon, basal, apical
    ...
    def set_type(self, index):
        if(index == 0):
            self.node_type = 'undefined'
        elif(index == 1):
            self.node_type = 'soma'
        elif(index == 2):
            self.node_type = 'axon'
        elif(index == 3):
            self.node_type = 'basal'
        elif(index == 4):
            self.node_type = 'apical'
    ...
    def set_type_from_name(self):
        if(self.node_type == 'undefined'):
            return 0
        if(self.node_type == 'soma'):
            return 1
        if(self.node_type == 'axon'):
            return 2
        if(self.node_type == 'basal'):
            return 3
        if(self.node_type == 'apical'):
            return 4
